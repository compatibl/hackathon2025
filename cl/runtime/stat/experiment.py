...
class Experiment(ExperimentKey, RecordMixin, ABC):
    ...
    def save_score(self, *, elapsed_sec: float, action_notice: str | None = None) -> None:
        ...
        num_successes = sum(1 for trial in trials if isinstance(trial, HackathonTrial) and trial.outcome)
        num_total = self.num_trials * num_cases
        num_remaining = num_total - num_completed
        score = 100 * float(num_successes) / float(num_completed) if num_completed > 0 else None

        # Clone experiment to save with updated fields
        experiment = self.clone()

        # Round score appropriately close to 100
        if score is not None:
            experiment.score = str(round(score, 2) if num_remaining == 0 else round(score, 0))
        else:
