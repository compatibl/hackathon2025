...
class LlmDraw(LlmDrawKey, RecordMixin):  # TODO: Requires refactoring after the introduction of LlmDrawKey
    ...
    def append_tokens(cls, tokens: Iterable[TPrimitive | None]) -> Self:
        ...
        if not tokens:
            ...
            return LlmDraw(trial_chain=previous_chain).build()
        else:
            ...
            if previous_chain:
                ...
                return LlmDraw(trial_chain=tuple([x for x in previous_chain] + serialized_tokens)).build()
            else:
                # Previous chain is empty, use serialized tokens as the entire chain
                return LlmDraw(trial_chain=tuple(serialized_tokens)).build()
    ...
    def _checked_token(cls, token: TPrimitive) -> None:
        ...
        if not is_primitive(token):
            ...
            if token == "":
                raise RuntimeError("An empty string is not a valid token for a LlmDraw.")
            elif "\n" in token:
                raise RuntimeError("A token for a LlmDraw cannot contain a newline character.")
            elif "\\" in token:
                raise RuntimeError(
                    "A token for a LlmDraw cannot contain a backslash character because\n"
