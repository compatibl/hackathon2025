...
class NumberParser(NumberReader):
    ...
    def read(self, text: str) -> NumberEntry:
        ...
        result = NumberEntry(text=text)

        # First non-AI text2num library to parse a number with suffix
        if (value := self._parse_number_with_suffix(text)) is not None:
            # Use parsed value
            try:
                ...
                raise ErrorUtil.value_error(
                    ...
                    data_type=NumberEntry,
                )
        else:
            # Try to parse a word description
            try:
                language, region = LlmSettings.parse_locale(self.locale.locale_id)
                value = text2num(text, language)
                result.value = float(value)
            except Exception as e:  # noqa
                raise ErrorUtil.value_error(
                    text,
                    details=f"Text description of a number could not be parsed using language code {self.locale.locale_id}.",
                    value_name="number",
